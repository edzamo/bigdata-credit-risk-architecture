...
- ✅ Diagnóstico del alcance ya realizado (ver `entregables/01-alcance-tecnico.md`).
...
- `kick-off-project.md` — mensajes de WhatsApp del revisor/tutor con el siguiente hito exigido: Docker funcionando + arquitectura medallón hasta capa Gold.
- `Plantilla de Proyecto de IEEE APR.docx` — documento de tesis (ya redactado en gran parte, no una plantilla vacía). Contiene el Capítulo I (diagnóstico) y Capítulo II (propuesta de arquitectura) ya definidos.
- `entregables/01-alcance-tecnico.md` — nota de alcance con el resumen del proyecto, la arquitectura de 6 capas ya especificada en el documento, confirmación de que **no hay nube definida** (todo apunta a on-premise/Docker según el kick-off), y las preguntas abiertas para el cliente.
...
Repositorio de ecosistema Big Data con arquitectura de medalla (Bronze / Silver / Gold), código modular en `src/`. **No crear estas carpetas todavía** — son el plan para cuando escribamos el primer módulo real (el hito de Docker + medallón hasta Gold). Evitamos scaffolding vacío sin lógica.
...
- **Storage medallón**: MinIO (compatible S3) sobre Docker — no está explícito en el documento, pero es la implementación natural del "Data Lake" que sí menciona, dado que todo apunta a on-premise (ver kick-off). A confirmar con el cliente (pregunta abierta #1 en `entregables/01-alcance-tecnico.md`).
